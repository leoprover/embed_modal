...
def main(qmltp_dir,out_dir,csv_file_list):
    ...
    bin_embed = [None]*3
    bin_embed[0] = "java -jar /home/tg/embed_modal/embed/target/embed-1.0-SNAPSHOT-shaded.jar" # dev
    bin_embed[1] = "java -jar /home/tg/oldemb/e_before_type/embed/target/embed-1.0-SNAPSHOT-shaded.jar" #_before_type
    bin_embed[2] = "java -jar /home/tg/oldemb/e_after_type/embed/target/embed-1.0-SNAPSHOT-shaded.jar" #_after_type
    ...
    for filename in filename_to_issue:
        ...
        for issue_dict in issue_list:
            ...
            sem = {"system":system,"quantification":quantification,"consequence":"$local","constants":"$rigid"}
            params = [None]*3
            params[0] = ["semantic_constant_quantification","semantic_cumulative_quantification","semantic_decreasing_quantification","semantic_modality_axiomatization"]
            params[1] = ["semantic_monotonic_quantification","semantic_antimonotonic_quantification","semantic_modality_axiomatization"]
            params[2] = []
            if issue_dict['quantification'] != "$varying":
                continue

            problemfile = filename_to_path(qmltp_dir,filename)
            print("currently processing",problemfile)
            with open(problemfile,"r") as fh:
                problem = fh.read()
                outOriginal = Path(out_dir) / (filename + "_original.p")
                with open(outOriginal,"w+")as fw:
                    fw.write(problem)
                for i in range(len(bin_embed)):
                    outfile = Path(out_dir) / (filename + "_vary_"+ str(i) + ".p")
                    if outfile.exists():
